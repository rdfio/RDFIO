			...
			switch ( $t['type'] ) {
				...
				case 'select':
					if ( in_array( $t['type'], array( 'select', 'describe' ) ) ) {
						...
						if ( isset( $t['result_vars'] ) )
							foreach ( $t['result_vars'] as $v ) {
								$string .= ' ?' . $v['var'];
							}
						if ( isset( $t['result_iris'] ) )
							foreach ( $t['result_iris'] as $v ) {
								$string .= ' <' . $v['iri'] . '> ';
							}
					}
				case 'ask':
				...
				case 'built_in_call':
					...
					foreach ( $t['args'] as $arg ) {
						$args[] = $this->sparql_info_to_string( $arg );
					}
					$string .= implode( ',', $args ) . ')  ';
					break;
				case 'var':
					$var_string = '?' . $t['value'];
					if ( isset( $t['direction'] ) )
						$var_string = strtoupper( $t['direction'] ) . "(" . $var_string . ")";
					$string .= $var_string;
					break;
				case 'literal':
				case 'literal1':
				...
				case 'expression':
					...
					foreach ( $t['patterns'] as $p ) {
						$expressions[] = $this->sparql_info_to_string( $p );
					}
					switch ( $t['sub_type'] ) {
						case 'relational':
